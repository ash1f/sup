...
class DropdownMenu extends Component {
    ...
    if (this.state.selectIndex[activityIndex] == index) {
      return (
        <View style={{ flex: 1, justifyContent: 'space-between', alignItems: "center", paddingHorizontal: 15, flexDirection: 'row' }} >
          ...
          <Icon fill={this.props.activityTintColor ? this.props.activityTintColor : this.defaultConfig.activityTintColor} name="checkmark-outline" height={20} width={20} />
        </View>
      );
    } else {
      return (
  ...
  renderDropDownArrow(index) {
    ...
    return (
      <Icon fill={this.props.tintColor ? this.props.tintColor : this.defaultConfig.tintColor} name="chevron-down-outline" height={20} width={20} />
    );
  }
        ...
        }} >
          {
            this.props.data.map((rows, index) =>
              <TouchableOpacity
                ...
                key={index}
                style={{ flex: 1, height: 50, alignItems: "center", justifyContent: "center", borderBottomWidth: 0.5, borderColor: "#999", paddingHorizontal: 15 }} >
